<script lang="ts">
	import ShareToSocialMediaSection from '$lib/components/challenge/share-to-social-media-section.svelte';
	import InviteFriendForm from '$lib/components/forms/invite-friend-form.svelte';

	import { headerState } from '$lib/stores/header-store';
	import { onMount } from 'svelte';
	headerState.set({
		backbutton: true,
		title: 'Freund einladen',
		hidden: false
	});

	let key;
	let ready = false;
	onMount(() => {
		key = randInt(0, 200);
		ready = true;
	});
</script>

<div>
	<svg
		xmlns="http://www.w3.org/2000/svg"
		fill="currentColor"
		viewBox="0 0 24 24"
		stroke-width="1.5"
		class="w-20 h-20 mx-auto p-4 text-water2"
	>
		<path
			stroke-linecap="round"
			stroke-linejoin="round"
			d="M15 19.128a9.38 9.38 0 002.625.372 9.337 9.337 0 004.121-.952 4.125 4.125 0 00-7.533-2.493M15 19.128v-.003c0-1.113-.285-2.16-.786-3.07M15 19.128v.106A12.318 12.318 0 018.624 21c-2.331 0-4.512-.645-6.374-1.766l-.001-.109a6.375 6.375 0 0111.964-3.07M12 6.375a3.375 3.375 0 11-6.75 0 3.375 3.375 0 016.75 0zm8.25 2.25a2.625 2.625 0 11-5.25 0 2.625 2.625 0 015.25 0z"
		/>
	</svg>
</div>
<h1 class="text-3xl text-center font-bold tracking-wide font-serif">Freund:in einladen</h1>
<p class="p-4 text-sm text-center font-serif">Zur zweit macht's mehr Spass!</p>

<!-- {#if ready} -->
<InviteFriendForm />
<!-- {/if} -->

<ShareToSocialMediaSection />
